...
import type { JobConnectionState } from '../app/api'
...
export function ExecutionStream({ job, events, connectionMode }: {
  job: Job | null
  events: EventItem[]
  connectionMode: JobConnectionState | 'idle'
}) {
  const connectionLabel = connectionMode === 'reconnecting' ? '重连中' : connectionMode === 'polling' ? '轮询' : connectionMode === 'connected' ? 'SSE' : '未订阅'
  return <div className="panel flex min-h-[326px] flex-col p-5 sm:p-6"><div className="flex items-start justify-between"><div><div className="eyebrow">02 / 执行流</div><h2 className="mt-2 text-xl font-semibold">实时执行轨迹</h2></div><div aria-live="polite" aria-atomic="true" className="flex items-center gap-1.5 rounded-full border border-[#28524b] bg-[#102b2a] px-2.5 py-1 font-mono text-[10px] text-[#8fe5c1]"><span className={`size-1.5 rounded-full bg-[#70e3ad] ${connectionMode === 'connected' ? 'animate-pulse motion-reduce:animate-none' : ''}`} />{connectionLabel}</div></div>{job ? <div className="mt-7 flex flex-1 flex-col"><div className="flex items-center justify-between border-b border-white/10 pb-4"><div><div className="font-mono text-[11px] text-[#6f9189]">{formatJobId(job.job_id)}</div><div className="mt-1 text-sm font-medium">{job.tool}</div></div><StatusBadge status={job.status} /></div><div className="mt-5 space-y-3">{events.slice(-4).map((event, index) => <div key={`${event.at}-${index}`} className="flex items-start gap-3 text-xs"><div className="mt-1.5 size-1.5 rounded-full bg-[#83e3bc] shadow-[0_0_12px_#83e3bc]" /><div className="min-w-0 flex-1"><div className="text-[#b2cbc4]">{event.detail}</div><div className="mt-1 font-mono text-[10px] text-[#7fa49c]">{event.at} · {event.status}</div></div></div>)}</div><div className="mt-auto flex items-center gap-2 pt-5 font-mono text-[10px] text-[#7fa49c]"><Clock3 size={13} />{job.status === 'completed' ? `完成于 ${formatTime(job.finished_at)}` : '等待状态更新…'}</div></div> : <EmptyStream />}</div>
}
