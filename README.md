...
任务执行支持两种后端：本地开发默认使用进程内线程池；Compose 默认启用 Redis 队列和独立 Worker。API 通过 `JOB_BACKEND=redis` 切换 Redis 模式，Worker 使用 `python -m src.worker` 启动，任务状态和队列可在多个 API/Worker 实例之间共享。
