...
def run_fixture(fixture_dir, paired_dir, output_dir, statistics_backend='scipy'):
    fixture_dir = Path(fixture_dir)
    paired_dir = Path(paired_dir)
    output_dir = Path(output_dir)
    output_dir.mkdir(parents=True, exist_ok=True)
    fastq_paths = sorted(fixture_dir.glob('*.fastq'))
    fastq_r2_paths = sorted(paired_dir.glob('*_R2.fastq'))
    if len(fastq_paths) != 6 or len(fastq_r2_paths) != 6:
        raise ValueError('expected six R1 and six R2 FASTQ fixture files')
    ...
    manifest_path = output_dir / 'rnaseq_paired_fixture_manifest.json'
...
def main():
    parser = argparse.ArgumentParser(description='Run the reproducible paired-end RNA-seq fixture.')
    root = Path(__file__).resolve().parents[1]
    parser.add_argument('--fixture-dir', default=root / 'examples' / 'omics' / 'rnaseq_fastq_fixture')
    parser.add_argument('--paired-dir', default=root / 'examples' / 'omics' / 'rnaseq_paired_fixture')
    parser.add_argument('--output-dir', default=root / 'output' / 'rnaseq_paired_fixture')
    parser.add_argument('--statistics-backend', choices=('scipy', 'auto'), default='scipy')
    args = parser.parse_args()
    result = run_fixture(args.fixture_dir, args.paired_dir, args.output_dir, args.statistics_backend)
