...
test('@mock 提交任务、查看结果并取消和重试任务', async ({ page }) => {
  ...
  await page.evaluate(() => localStorage.setItem('artifact-xss-sentinel', 'safe'))
  await previewButton.click()
  const dialog = page.getByRole('dialog', { name: 'HTML 报告预览' })
  await expect(dialog).toBeVisible()
  const previewFrame = dialog.locator('iframe')
  ...
  await expect.poll(() => page.evaluate(() => localStorage.getItem('artifact-xss-sentinel'))).toBe('safe')
  await expect.poll(() => page.evaluate(() => document.body.dataset.artifactXss || '')).toBe('')
